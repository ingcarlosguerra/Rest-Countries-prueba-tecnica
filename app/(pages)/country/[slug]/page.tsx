...
export default async function CountryDetails({
  ...
  const Property = ({ label, value, col = "1" }: PropertyProps) =>
    value ? (
      ...
      <div className="max-w-5xl mx-auto px-4 lg:px-0 py-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 py-6 gap-8">
          ...
          <div className="grid sm:grid-cols-2 h-min my-auto py-6 gap-2">
            ...
            <Property label="Top Level Domain:" value={country.flag} />
            ...
            {country.borders && (
              <div className="text-gray-400 dark:text-gray-300 sm:col-span-2 flex flex-col sm:flex-row space-x-4">
                <p className="font-semibold text-nowrap py-2">
                  Border Countries
                </p>
                <div className="flex flex-wrap gap-2">
                  {country.borders.map((name) => (
                    <span
                      key={name}
                      className="flex items-center gap-2 py-2 px-6 rounded-md bg-gray-200 text-gray-600 dark:text-gray-300 dark:bg-gray-700 w-fit shadow-sm shadow-gray-950/20"
                    >
                      {name}
                    </span>
                  ))}
